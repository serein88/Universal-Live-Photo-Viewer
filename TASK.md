...
| T4   | Windows MVP 查看器                               | 完成  | 完成目录扫描、分页查看与双层播放基础交互                                                   | 1) 可浏览并播放 V1 样本；2) 关键桌面交互可复现                                              | 设计稿 §3/§6               | 母任务（由子任务驱动）       |
| T4-1 | Windows MVP 查看器-工程初始化                         | 进行中 | 配置 Windows 工程与窗口基础能力                                                   | 1) 可启动运行；2) 窗口能力生效                                                        | Roadmap Phase 2         | 云端构建复测中（修复 release 编译阻塞） |
| T4-2 | Windows MVP 查看器-目录扫描与列表                       | 完成  | 实现“选目录 -> 递归扫描 -> 列表渲染”                                                | 1) 可展示样本列表；2) 支持刷新重扫                                                      | Roadmap Phase 2         | T4 子任务            |
| T4-3 | Windows MVP 查看器-双层播放器                         | 完成  | 实现静图底层 + 视频上层播放链路                                                      | 1) 切换与播放可用；2) 播放结束回退静图                                                    | Roadmap Phase 2         | T4 子任务            |
| T4-4 | Windows MVP 查看器-桌面交互                          | 完成  | 实现滚轮切图与键盘左右切换                                                          | 1) 两种交互可用；2) 焦点切换稳定                                                       | Roadmap Phase 2         | T4 子任务            |
