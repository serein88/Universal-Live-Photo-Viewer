...
| T4-5 | Windows MVP 查看器-Windows Build 冒烟测试           | 待确认 | 验证 GitHub Actions 构建产物可下载、可解压、可运行                                      | 1) `workflow_dispatch` 构建成功；2) 本地运行非 Demo 主界面；3) 基础播放链路可观察                          | 当前会话                    | 已修复 Windows 播放异常，待你重新下载构建包回归验证 |
