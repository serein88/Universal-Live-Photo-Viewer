...
| T11  | GitHub 安全基线加固（令牌/分支/环境）                        | 待确认 | 落地仓库协作安全最小权限方案并补齐安全操作手册                                               | 1) CI 权限最小化；2) 发布作业绑定受保护环境；3) `docs/` 新增安全操作手册                         | 当前会话                    | 已补充手册与发布环境工作流，待仓库设置确认 |
...
| T4-5 | Windows MVP 查看器-Windows Build 冒烟测试           | 待确认 | 验证 GitHub Actions 构建产物可下载、可解压、可运行                                      | 1) `workflow_dispatch` 构建成功；2) 本地运行非 Demo 主界面；3) 基础播放链路可观察                          | 当前会话                    | 已修复注册名编译错误与播放异常，待你重跑 Actions 回归验证 |
| T4-6 | Windows MVP 查看器-Windows 本地冒烟标准化脚本       | 待确认 | 新增本地 PowerShell 冒烟脚本与执行文档，固化最小验证路径与日志格式                                | 1) `tool/windows_smoke_check.ps1` 可生成结构化日志；2) `docs/testing/windows-local-smoke.md` 覆盖执行与上传要求；3) 固定字段可用于自动比对 | 当前会话                    | 本轮已完成脚本与文档，待你在 Windows 本地执行并回传日志/截图 |
| T4-7 | Windows MVP 查看器-预览区翻页控件与序号提示         | 失败   | 在预览区补充上一张/下一张按钮与当前序号显示，降低仅键盘/滚轮操作门槛                                | 1) 预览区显示 `当前序号/总数`；2) 上一张/下一张按钮可切换选中项；3) 现有键盘/滚轮切换行为不回归 | 当前会话                    | 本轮阻塞：云端外网被 403 拦截，无法安装 Flutter/Dart；建议直接用 GitHub Actions 完成云端验证 |
